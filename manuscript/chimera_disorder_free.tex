...
The stability of this DTC phase exhibits notable disparities between strong and weak coupling interactions. A the CDT/DL point, it is seen that the DTC phase exhibits enhanced stability in the presence of strong coupling compared to weak coupling, specifically for interactions that are not long-range in nature. The phenomenon of stable DTC is observed in long-range spin chains, regardless of the strength of spin interactions. Therefore, it is feasible to see the emergence of a chimera state consisting of two separate phases of matter, specifically a time crystal and a localized ferromagnet, simultaneously in a spin-chain system. This phenomenon can occur under typical experimental settings, with long-range interactions being the most suitable candidate.



\red{ In order to gain a comprehensive understanding over selection of long range interaction, we have conducted a numerical investigation of the Fast Fourier Transform (FFT) applied to the local magnetization at randomly selected spin $(site(i)=1)$ in regions A. The results of this analysis are visually presented in Fig.\ref{Fig:sz_single}. The frequency data has been normalized by dividing the frequency $\omega$ by the driving frequency (say, $\omega_{_{D}}=20$). Several prominent peaks at half integer multiples of $\omega/\omega_{_{D}}$ are detected. The subharmonic peaks at $\omega/\omega_{_{D}} = 0.5n; n= 1,3,5,\dots$ breaks the time translational symmetry. This is detected for each of the spin ranges defined by $\beta$'s. There are a few other peaks present in the plot for both of the weak and strong coupling interaction. In weak spin-spin coupling, these extra Fourier modes affects the stability of DTC.  However, at $\beta=0$, the additional peaks are submissive with respect to the subharmonic peaks. Also, the subharmonic peak at $\omega/\omega_{_{D}}=1/2$ dominates over others. Thus at low coupling, long range is preferable. In case of the strong coupling interaction, the spin-spin interaction can surplus these Fourier modes and only the subharmonic response at $\omega/\omega_{_{D}} = 0.5$ dominates.}
...
\end{figure}
	...
	We expanded the numerical simulations outlined in  sec.~\ref{sec:level3} to encompass longer duration. Consequently, we acquired long-time estimates of the regional magnetization for regions A and B under varying conditions of spin coupling strength and range. The results are plotted in Fig.~\ref{Fig:regiogionalmag}. In both instances, the drive parameters were set to the CDT/DL point. In region B, the value of $M^Z_B$ remains constant at unity across all time for all ranges. In contrast, the behavior of $M^Z_A$ in region A is distinct. In the regime of weak coupling, the persistence of $M^Z_A$ is significant solely for all-to-all interactions, characterized by $\beta=0$. However, for interactions at other ranges, $M^Z_A$ gradually dissipates with time. In the regime of high coupling, it can be observed that the DTC phase present in region A undergoes a gradual dissolution over time, regardless of the range of spin interactions.\red{ The numerical result can be understood from the FFT of regional magnetization plot in Fig\ref{Fig:regionalFFT}. At low coupling strength at only $\beta=0$, the subharmonic peaks at $\omega/\omega_{_{D}}=0.5$ domiantes over other frequency responses, there by a steady regional magnetization over a large time. However when $\beta$ increases, the additional Fourier modes start interplay and $M^Z_A$ starts devolve after $~50 T$. Yet in case of strong coupling, the spin-spin interaction dominates over Fourier modes. Thus the regional magnetization at strong coupling requires longer time than weaker coupling to melt. }
...
In contrast to the preceding subsection, which focused on the examination of macroscopic observables, the current section delves into a more direct analysis of microscopic aspects through the consideration of entanglement entropy (EE). The entanglement entropy (EE) is commonly employed as a measure to quantify the degree of entanglement exhibited by quantum states across invariant subspaces. It provides valuable insights pertaining to quantum correlations and the ability for information storage. The mathematical expression for the entanglement entropy, denoted as $S_{AB}$, between areas A and B, may be described using the von Neumann entropy~\cite{bayat_entanglement_2022,mendes-santos_measuring_2020}.   The density matrix of a pure state $\ket{\psi}$ is $\hat{\rho}= \ketbra{\psi}$. The reduced density matrices (RDMs) for regions A and B, denoted as $\hat{\rho}_A$ and $\hat{\rho}_B$ respectively, are obtained by taking the partial traces of $\hat{\rho}$ with respect to the complementary regions. Specifically, $\hat{\rho}_A$ is obtained by tracing out region B, \textit{i.e.}, $\hat{\rho}_A\equiv \Tr_B(\hat{\rho})$, while $\hat{\rho}_B\equiv\Tr_A(\hat{\rho})$. The entanglement entropy (EE) is determined by
	...
	\todo[inline]{I'm going to stop here. The expression below is totally wrong, and does not agree with equations 2 and 3 of Sen \textit{et. al.}.  There, the series is $H_F = \sum_n T^n H^{(n)}_F$ and the coefficients of the terms are all different. Please correct them and adjust all subsequent calculations to match. I cannot trust the rest of the calculations until the coefficients are corrected. Also, please use the physics package from \LaTeX\;for commutators and other quantum expressions.}
	\begin{align}
