...
\begin{document}
	...
	\begin{equation}
		...
		\label{eq:rot1}
	\end{equation}
	\blue{$\mathcal{T}$ is the time ordering operator}. The unitary transformation operator \blue{$\hat{U}(t)$} \sout{for } \blue{corresponding to} the Hamiltonian $\hat{H_2}$ for the time interval $T_2$, $\in{\Big[\frac{T}{2}, (\frac{T}{2}+t) \Big]}$ is,
	\begin{align}
	...
	\begin{align}
		...
		\label{eq:movham}
	\end{align}
	We have applied Jacobi Anger expansion, $\displaystyle e^{iz \cos(\omega t)} = \sum_{n=-\infty}^{\infty} \mathcal{J}_n(z) e^{in\omega t}$, where $\mathcal{J}_n$'s denote the roots of the Bessel's function of first kind of $n$'th order. \blue{We introduce Rotating Wave Approximation (RWA) which at high frequency limit, approximately equates terms in Eq.\eqref{eq:movham} with their coarse-grained values. This results in averaging all the harmonic oscillating terms to be 0, except for zero'th order term (see Appendix\ref{AppendixA}). Thus considering only the zeroth order terms in Jacobi-Anger expansion, the moving frame Hamiltonian $\hat{H}^{mov}$ reduces to,
		\begin{multline}
	...
	The time evolution operator \blue{$\hat{U}(t)$} is found to be \cite{haldar_statistical_2022},
	\begin{equation}
	...
	\blue{We consider, $\hat{H}_2(t) = \hat{H}_{20} + \hat{H}_{21}(t)$.} Now, the Hamiltonian in moving frame can be transformed to\cite{haldar_statistical_2022, haldar_dynamical_2021},
	\begin{align}
		\hat{H}^{mov} &= \blue{\hat{U}}^\dagger(t) \hat{H}(t) \blue{\hat{U}}(t)- i \blue{\hat{U}}^\dagger(t) \partial_t \blue{\hat{U}}(t) \nonumber\\
		&= \blue{\hat{U}}^\dagger(t) \blue{\hat{H}_{20}} \blue{\hat{U}}(t)\nonumber\\
		...
		&= \hbar\sum_{ij} J_{ij} \Big(\hat{\sigma}^y_i\hat{\sigma}^y_j\Big) \Big(\blue{\mathbbm{1}}\cos^2(2\zeta) -\hat{\sigma}^z_i\hat{\sigma}^z_j \sin^2(2\zeta) + i (\hat{\sigma}^z_i + \hat{\sigma}^z_j)\cos(2\zeta)\sin(2\zeta)\Big).
		\label{eq:hmovap1}
	\end{align}
	...
	Putting these values in the Floquet operator in \blue{ Eq.\eqref{eq:floqop0}} can be written as,    	
	\begin{align}
		\hat{\mathcal{F}}^2 
	...
	\end{align}
	\blue{
		We apply Baker-Campbell-Hausdorff formula \cite{Scharf_1988} and approximate the series as given by,
		\begin{equation}
			\exp(\alpha \hat{A}) \exp(\beta \hat{B}) \approx \exp(\alpha \hat{A} + \beta\hat{B} + \frac12 \comm{\hat{A}}{\hat{B}}),
		\end{equation}
		Thus one can write\cite{sakurai_phys_nodate},
		\begin{align}
			...
			\label{eq:bch}
		\end{align}
		This is valid when $\alpha$ and $\beta$ are small. In our proposed model, the spin-spin coupling terms and rotational error($\epsilon_A$) are small. So we can utilize the above Eq.\eqref{eq:bch} to simplify the Eq.\eqref{eq:floqop1} as follows,
	}
