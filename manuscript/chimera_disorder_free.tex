...
\begin{document}
	...
	\begin{align}
		...
		\label{eq:movham}
	\end{align}
	We have applied Jacobi Anger expansion, $\displaystyle e^{iz \cos(\omega t)} = \sum_{n=-\infty}^{\infty} \mathcal{J}_n(z) e^{in\omega t}$, where $\mathcal{J}_n$'s denote the roots of the Bessel's function of first kind of $n$'th order. \blue{We introduce Rotating Wave Approximation (RWA) which at high frequency, approximately equates $e^{i\frac{4h}{\omega}\cos(\omega t)}$ terms in Eq.\eqref{eq:movham} with its coarse-grained value. This averages all the harmonic oscillating terms to 0, except for zero'th order term (see Appendix\ref{AppendixA}). Thus considering only the zeroth order terms, the moving frame Hamiltonian $\hat{H}^{mov}$ reduces to,
		\begin{multline}
	...
	\begin{align}
		...
		\label{eq:hmovap1}
	\end{align}
	Here, we have utitlized the expression, $\exp[ia(\hat{n}\cdot\vec{\sigma})] = \mathbbm{1} \cos(a) + i (\hat{n}\cdot\vec{\sigma})\sin(a)$. 
	
	We invoke the Jacobi-Anger expansion $\displaystyle e^{iz\cos(\omega t)} = \sum_{n=-\infty}^{\infty}\mathcal{J}_n(z)e^{in\omega t}$\cite{arfkenmath}, where $\mathcal{J}_n(z)$ is the Bessel funciton of the first kind of n'th order. The drive parameters $h$ and $\omega$ are independent. This allows to coarse-grain average on the term $e^{i\frac{4h}{\omega}\cos(\omega t)}$ in Eq.\eqref{eq:hmovap1} such that, 
	\begin{equation}
		\expval{e^{i\frac{4h}{\omega}\cos(\omega t)}}_t = \sum_n \mathcal{J}_n e^{in\omega t}\expval{e^{in\omega t}} = \mathcal{J}_0\left(\frac{4h}{\omega}\right); \forall \left(\frac{4h}{\omega}\right) \in \mathbb{R}.
	\end{equation}
	The coarse-graining averages all the harmonic oscillations to zero, and only $n = 0$'th term survives. When $\omega \gg1$, Rotating Wave Approximation (RWA) yields, $ e^{i\frac{4h}{\omega}\cos(\omega t)} \approx \expval{e^{i\frac{4h}{\omega}\cos(\omega t)}}_t = \mathcal{J}_0\left(\frac{4h}{\omega}\right)$. Thus considering only $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$ terms in Eq.\eqref{eq:hmovap1}, the $\hat{H}^{mov}$ reduces to RWA-Hamiltonian ($\hat{H}^{_{RWA}}$) as given by,
	\begin{equation}
