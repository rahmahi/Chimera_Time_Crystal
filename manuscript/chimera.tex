...
The phenomenon of a \textit{ chimera state} is observed in coupled systems of  identical oscillators, when spontaneous synchronized and desynchronized dynamics coexist simultaneously~\cite{kuramoto_coexistence_2002, panaggio_chimera_2015}. Kuramoto et al. first detected this phenomena in a network of non-locally coupled phase oscillators in 2002. Two domains of coherent oscillations with unique frequencies and incoherent oscillations with distributed frequencies were observed.~\cite{kuramoto_coexistence_2002}. Chimeras have been widely explored in classical systems in the last decade\cite{parastesh_chimeras_2021,chimera_book, taniya2022}. The origin of chimera lies in symmetry-breaking bifurcation in the Kuramoto model which leads to a breakdown of global synchronization, giving rise to a chimera state where spatially distinct regions exhibit different synchronization behaviors\cite{Kotwal2017}. The coexistence of synchronized and desynchronized states in a chimera state can be considered a manifestation of spontaneous symmetry breaking in the context of nonlinear dynamics\cite{Aneta2013}. In physical realm, chimera states serves as a possible explanation of Unihemispheric Slow Wave Sleep (UHSW) in migrating birds, seals and domestic chicks \cite{Rattenborg2000, Rattenborg2006, Rattenborg2016}. Studies on chimera has proposed the control over macroscopic systems like power-grid where synchronous state is stable via tunable parameters of the generator\cite{Motter2013}. 
...
\begin{align}
    ...
    \label{eq:hmovap1}
\end{align}
In the penultimate step, we used the Baker-Campbell-Hausdorff formula. Now, we invoke the Jacobi-Anger expansion, $e^{i\eta \cos(\theta)} = \sum_{n=-\infty}^{\infty}\mathcal{J}_n(\eta)e^{in\theta}$, where $\mathcal{J}_n(z)$ is the Bessel function of the first kind of $n^{th}$ order, and apply to the RHS of  Eq.~\ref{eq:hmovap1}. This produces a Fourier series expansion for each amplitude in
...
\label{eq:sqr}
\end{align}
Finally, we substitute the values from Eq.\eqref{eq:sqr} into Eq.\eqref{eq:hmovap1}. This reduces the moving frame Hamiltonian into a RWA-Hamiltonian ($\hat{H^{_{RWA}}}$) given below,
\begin{multline}
    ...
    \label{eq:hrwa}
\end{multline}	
Now, for a particular $\omega$, if $h$ is controlled in such fashion that $\frac{4h}{\omega}$ lies on one of higher the roots of $\mathcal{J}_0\Big(\frac{4h}{\omega}\Big)$, then $\hat{H}^{_{RWA}}$ becomes the "Frozen Hamiltonian",
\begin{equation}
\hat{H}^{_{RWA}}_{frz} = \frac{\hbar}{2}\sum_{ij} J_{ij}  \Big[\hat{\sigma}^x_i\hat{\sigma}^x_j + \hat{\sigma}^y_i\hat{\sigma}^y_j\Big].
\label{eq:hrwa:frz}
\end{equation}	
...
\end{align}
maintains the locality of the Hamiltonian in the basis of fermionic creation and annihilation operators $\hat{c}^{\;}_i, \hat{c}^\dagger_i$ \cite{mbeng2020}. Thus, the Frozen Hamiltonian transforms to the standard tight-binding Hamiltonian $\hat{H}^{_{RWA}}_{frz}=(J_0/2)\sum_i \hat{c}^\dagger_i \hat{c}^{\;}_{i+1}$. The system is completely localized due to the presence of an infinite number of local (approximate) Noether Charges, given by $\hat{n}_m= \hat{d}^{\dagger}_m\hat{d}^{\;}_m$, where
\begin{equation}
\hat{d}_m = \frac{1}{\sqrt{N}}\sum_{n=0}^{N-1}e^{-\frac{2nm\pi i}{N}}\hat{c}_n.
\end{equation}
When the $J_{ij}$s are extended beyond the nearest-neighbour case to longer ranges, the locality of the Jordan Wigner transformation is destroyed. Nonetheless, both fully polarized states $\ket{\uparrow\uparrow \dots\uparrow}$ and  $\ket{\downarrow\downarrow \dots\downarrow}$ remain eigenstates of $\hat{H}^{_{RWA}}_{frz}$, and their dynamics are still approximately localized in the $T_2$ time interval. In the opposite limiting case of all-to-all interactions in the spins, where $J_{ij}=J_0 \;\forall\;i,j$, we can write $\hat{H}^{_{RWA}}_{frz}\sim \mathbf{S}^2 - \left(S^z\right)^2$, where $\mathbf{S} = \left(S^x, S^y, S^z\right)$, and $S^\mu \sum_i {s}^\mu_i$. In that case, any eigenstate of the macroscopic spin, $\ket{S, S_z}$, is dynamically localized in the $T_2$ time interval. 

In our driving protocol, the system is always ideally populated in a product state at the end of every $T_1$ time interval, with region A being polarized and region B polarized in the opposite direction. Now, as per the Buyskikh classification scheme ~\cite{buyskikh_entanglement_2016}, we have established complete localization in the \textit{nearest-neighbor} and \textit{all-to-all} cases. Some dynamics is expected for intermediate and short ranges due  the contributions of cross-terms between the two regions in $\hat{H}^{_{RWA}}_{frz}$. Nonetheless, these contributions are expected to be minimal in the thermodynamic limit, both for short ranges (due to the area-volume law), and intermediate-ranges (due to minor deviations from the all-to-all case). Numerical analysis reveals that the dynamics of the driven system retains considerable  localization in the $z-$correlations of the spins for most intermediate and short-range cases.\todo[inline]{Describe the numerics here.}
