...
\begin{figure}[t]
	\centering
	\hspace{1.5cm}\includegraphics[width = 15.5cm]{figure8.pdf}
	\caption{\red{Temporal changes in regional magnetization ($M^z_{A/B}$) of the spin-1/2 chain for various spin rotational errors ($\epsilon_{_{A/B}}$) in different panels for  several ranges of spin interactions (characterized by $\beta$ as specified in the legends). The time period is fixed by setting the drive parameters $h,\omega$ to the CDT/DL point, \textit{i.e.} one of the root of $\mathcal{J}_0(4h/\omega)$. We have considered a constant rotational error $\epsilon_A = 0.03$ and varied $\epsilon_B(= 0.1, 0.3, 0.7,0.9$) and plotted variation of $M^z_{A/B}$  in left side panels namely `a',`b',`c',`d' respectively.  Additionally we have considered constant rotational error $\epsilon_B = 0.1$ and varied $\epsilon_A ( = 0.03, 0.4,0.7$) and plotted variation of $M^z_{A/B}$ in bottom panels namely `d',`e' and `f' respectively. Furthermore a set of $\epsilon_{A,B}$ which is $ (\epsilon_{A},\epsilon_{B}) = \{(0.1,0.11), (0.3,0.31),(0.5,0.51),(0.8,0.81))\}$ is considered ensuring $\epsilon_A\sim \epsilon_B$ and plotted variation of $M^z_{A/B}$  in panels namely `g',`j',`i' and `h' respectively. }}
