...
\begin{document}	
\title{Time Crystal Embodies Chimera\red{like state} in Periodically Driven Quantum Spin System}
\submitto{\NJP}
...
\begin{figure}[t!]
	\centering
	\hspace{1.5cm}\includegraphics[width=13.5cm]{figure7.pdf}
	\caption{\red{The time evolution of local magnetization for each spin at region A (i=0,1,2,3) and region B (i=4,5,6,7) is plotted for $N=8$ spins in a periodically driven one-dimensional spin chain, starting from a fully polarized spin state, with drive frequency $\omega=20$. The strong spin-spin coupling $J_0 = 0.2/T$, spin rotation error $\epsilon_A = 0.03$ and $g=\pi/T$, where $T=2\pi/\omega$ is the time period of the drive.  Different ranges of spin-interactions such as long-range($\beta=0$), intermediate range($\beta=1.5$), short range($\beta=2.5$), and nearest neighbor range($\beta=\infty$), are chosen and plotted  for $\epsilon_B =0.6$[(a)-panel] and $\epsilon_B =0.8$[(b)-panel], respectively, from left to right, namely (a, b, c, d) in each panel.}}
	\label{Fig:eb}
\end{figure}
\red{This observation holds true in cases where the spin rotational error $\epsilon_B$ is significant ($\sim 0.9$), while $\epsilon_A$ remains relatively small ($\le 0.1$).}

\red{To investigate the impact of $\epsilon_B$ on the emergence of chimeralike state, we have considered the spin-1/2 chain with all sites initially populated with up-polarized spins. We have considered strong spin-coupling interaction ($J_0 = 0.2/T$) with drive frequency $\omega = 20$ and an amplitude corresponding to a CDT/DL point, maintaining constant $\epsilon_A=0.03$. We have numerically calculated and plotted the temporal variation of local magnetization in figure~\ref{Fig:eb} for different interaction ranges at each site (i) for two values of $\epsilon_B$ which is $0.6$ and $0.8$. The DTC is found to melt faster as we decrease values of $\epsilon_B$. Additionally when the spin-spin interaction range (defined by $\beta$) increases the relative rate of melting of DTC increases for a corresponding $\epsilon_B$. Moreover we observe	that the rate of DTC-melting corresponding to the each of the ranges of interaction, increases as $\epsilon_B$ decreases.
}\todo{next write DMBL melts and the inter relation between DMBL and stability of DTC}
