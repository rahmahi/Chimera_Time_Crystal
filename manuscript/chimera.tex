...
\begin{figure}[t]
	...
	\caption{\red{Regional magnetization $M^z_{A/B}$ of the spin-chain for various spin rotational errors ($\epsilon_{_{A/B}}$) in different panels for  different ranges of spin interactions (characterized by $\beta$ as specified in the legends). The time period is fixed by setting the drive parameters $h,\omega$ to the CDT/DL point, \textit{i.e.} one of the root of $\mathcal{J}_0(4h/\omega)$. We have considered a constant rotational error in A, $\epsilon_A = 0.03$ and varied $\epsilon_B(= 0.1, 0.3, 0.7,0.9$} and ploted in panels a,b,c,d respectively.  Additionally we have considered rotational error $\epsilon_B = 0.1$ and varied $\epsilon_A ( = 0.03, 0.4,0.7$ in panels d,e,f. Furthermore we have considered a set of  }
