...
\end{equation}
\red{where, $\hat{\Phi}$ is a local operator, $\hat{\Psi(nT)}$ is the state of the system at time $nT$ and n>1~\cite{else2016,Guo_2020}. Thereby the system exhibits a subharmonic response to the original drive frequency as ($\omega/n$). Thus DTTS breaks down. Here `short range correlated' state $\ket{\psi}$ is defined for a local operator $\Phi(\vec{x})$, }

\begin{equation}
    \expval{\hat{\phi}(\vec{x})\hat{\phi}(\vec{x'})}{\psi} \rightarrow \expval{\hat{\psi}(x)}{\psi}\expval{\hat{\psi}(x^\prime)}{\psi},
\end{equation}
\red{as $\abs*{x-x^\prime}\rightarrow 0$. This is called cluster decomposition.}
