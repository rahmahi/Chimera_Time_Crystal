...
\begin{figure}[t]
	...
	\end{center}
	\caption{FFT over regional magnetization $M^z_A$ data for upto $8\times10^4$T for different spin interaction ranges($\beta = 0,1.5,2.5, \infty[`inf']$) at panels top to bottom respectively. Weak (left panels) and strong (right panels) spin coupling is employed. The drive parameters are at the CDT/DL point considering constant $\omega = 20$ with time period $T= 2\pi/\omega$. The beat frequency $\delta\Omega_B$ is plotted in inset figure of each panel for different system size N.}
	\label{Fig:size_dependence}
\end{figure}	
We observed melting in DTC phase for all possible spin interaction in strong coupling as well as for all selected short range interactions ($\beta > 0$) in weak coupling in figure~\ref{Fig:regiogionalmag}. This necessitates an inquiry over the stability of DTC phase at otherwise system sizes. The DTC-melting accompanies beats in time evolution curves in $M^z_A$ in system size N=8. 

We have considered a set of  different system sizes N = 4,6 and 10 including N=8 and numerically calculated the temporal variation of for $M^z_A$ for an extreme extended time period (up to $8\times10^4$T). We have performed numerical FFT using QuTip~\cite{Johansson2013} and plotted the obtained values for different spin interaction ranges (defined by $\beta$) and coupling strengths in figure~\ref{Fig:size_dependence}. The existing peaks in FFT plots in each panels denote existence of all available frequencies which contributes to the system dynamics over large time. The priority in frequencies is secured by the corresponding amplitudes. We have numerically obtained the beat frequency ($\delta\Omega_B$) from the most prominent frequencies around $\Omega/\omega = 1/2$ and plotted it for different system size N(=4,6,8,10) in the inset of each panel. We observe that $\delta\Omega_B$ decreases with the increase in N at each panel. This indicates that $\delta\Omega_B$ is finite at finite system size. However at thermodynamic limit which is when $N\rightarrow \infty$, $\delta\Omega_B$ is found to tend to zero ($\delta\Omega_B\rightarrow 0$) which results in collapse in FFT-spectrum to a single peak at $\Omega/\omega = 1/2$. This asserts that the stability increases in proposed finite size spin-1/2 model is enduring by increase in N for all selected spin interaction ranges and coupling strengths. And at thermodynamic limit DTC-DMBL chimeralike order is stable at each possible interaction ranges and coupling strengths.
...
\end{figure}
The influence of the driving parameters $h, \omega$ significantly affects the long-lasting stability of the DTC-DMBL chimera\red{like order}. The stability of the chimera\red{like order sustains at minor deviation from CDT/DL point; however at significant deviation chimeralike order} diminishes. \sout{even if there is a minor} \sout{deviation from the CDT/DL point} \red{(see ~\ref{sec:AppendixC})}. This raises questions about the mechanisms by which the chimera\red{like order} resists external fields, even while functioning precisely at the CDT/DL point.
...
\begin{figure}[t]
	\begin{center}
		\includegraphics[width=10cm]{figure14.pdf}
	\end{center}
...
\red{We have considered the spin-1/2 chain as described by the equation~\ref{eq:sysham2}. The drive frequency is considered at fixed high value which is $\omega=20$ and the amplitude ($h$) is corresponding to the CDT/DL point. We introduce a small deviation $\Delta_h$ to $h$. We numerically calculate the fidelity ($F_n$) of the system. It measures the closeness of a state of a system to the state over time. Fidelity is defined as $F_n = \abs{\braket{\psi(0)}{\psi (2nT)}}^2$ ~\cite{Jozsa1994,Liu2023}, where $\psi(t)$ denotes the wave function of the system at time `$t$' and `$n$' is the number of period doubling. We considered $n=10$, $\beta=0$ and selected all-up polarized initial state. We have numerically obtained $F_n$ for several $\Delta_h$'s and plotted in figure~\ref{Fig:aroundCDT} for different system size N=6,8,12. The fidelity is found to be sufficiently high around CDT/DL point ($\frac{4\Delta_h}{\omega} = 0$) in gray color-filled region in the figure. However when the deviation $\Delta_h$ is significant, fidelity decreases manifesting melting chimeralike order.  This proclaims the stability of the system at CDT/DL point as well as minor deviation from it.}
