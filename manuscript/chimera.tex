...
\red{Furthermore in the bottom panels `d',`e',`f' we have considered a constant $\epsilon_{B} = 0.1$ and varied $\epsilon_{A}(=0.03,0.4,0.9)$ respectively. When $\epsilon_{A}=0.03~\&~ \epsilon_{B}=0.1$, in panel-(d) we observe simultaneous melting of DTC phase in region A and B. With further increasing $\epsilon_{A}$, from the melting of DTC in region-A a transition towards DMBL is observed. Additionally, the stability of DTC phase in region-B increases with increase in $\epsilon_{A}$. Thus at significantly small $\epsilon_{A}$ and large $\epsilon_{B}$ results in a DTC phase in region-B and a DMBL phase in region-A, manifesting a chimera like order. Therefore \dots\dots}
