...
\begin{figure}[t]
	...
	\caption{\red{Regional magnetization $M^z_{A/B}$ of the spin-chain for various spin rotational errors ($\epsilon_{_{A/B}}$) in different panels. The magnetization of regions A (red) and B (blue) is plotted as functions of time $t/T$ for both weak spin coupling ($J_0=0.072/T=0.229$), left) and strong spin-coupling ($J_0=0.2/T=0.637)$, right) of each of the panels and different ranges of spin interactions (characterized by $\beta$ as specified in the legends). The time period is fixed by setting the drive parameters $h,\omega$ to the CDT/DL point, \textit{i.e.} one of the root of $\mathcal{J}_0(4h/\omega)$. We have considered a constant $\epsilon_A = 0.03$ and varied $\epsilon_B(= 0.1, $}}
	\label{Fig:reg_mag_ea_eb}
\end{figure}\todo{write from the caption of fig 8}
