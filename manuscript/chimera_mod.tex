...
Next, we apply the \textit{Jacobi Anger expansion}, $\displaystyle e^{iz \cos(\theta)} = \sum_{n=-\infty}^{\infty} i^n \mathcal{J}_n(z) e^{in\theta}$, where $\mathcal{J}_n$' s denote the Bessel function of first kind of $n^{th}$ order. This allows for a Fourier series expansion of $\hat{H}^{mov}(t)$. Now, we introduce the Rotating Wave Approximation (RWA), which, in the high-frequency limit, approximates the Fourier modes with their coarse-grained values over long times. This allows us to average out all but the zeroth Fourier mode. \blue{Additionally, we consider the asymptotic form of Bessel function for larger variables, which can be appromiated to neglect the higher order Bessel functions (see~\ref{sec:AppendixA} for details). This results in the approximation,}
\begin{multline}
...
\begin{align}
    ...
    \label{eq:hmovap1}
\end{align}
Here, we have utilized the expression, $\displaystyle \exp[ia(\hat{n}\cdot\vec{\sigma})] = \mathbbm{1} \cos(a) + i (\hat{n}\cdot\vec{\sigma})\sin(a)$. In the high-frequency limit, where $\omega \gg J_0$, the Rotating Wave Approximation (RWA) allows for approximating the Fourier modes of a time-periodic Hamiltonian in the moving frame with their long-time averages. We can obtain the Fourier modes by applying the  Jacobi-Anger formula, $e^{i\eta \cos(\theta)} = \sum_{n=-\infty}^{\infty}i^n \mathcal{J}_n(\eta)e^{in\theta}$ to the RHS of Eq.~\ref{eq:hmovap1}. Here, $\mathcal{J}_n(z)$ is the Bessel function of the first kind of $n^{th}$ order. Averaging over all times yields the following approximations.
\begin{align}
2\cos^2(2\zeta) &\approx  \mathcal{J}^2_0\Big(\frac{4h}{\omega}\Big) + 2\sum^\infty_{n=1} \mathcal{J}^2_n\Big(\frac{4h}{\omega}\Big)+ \cos\left(\frac{4h}{\omega}\right)\mathcal{J}_0\Big(\frac{4h}{\omega}\Big)  \label{eq:cossqr2}\\
2\sin^2(2\zeta) &\approx  \mathcal{J}^2_0\Big(\frac{4h}{\omega}\Big) + 2\sum^\infty_{n=1} \mathcal{J}^2_n\Big(\frac{4h}{\omega}\Big)- \cos\left(\frac{4h}{\omega}\right)\mathcal{J}_0\Big(\frac{4h}{\omega}\Big)  \label{eq:sinsqr2}\\
\sin(2\zeta) \cos(2\zeta) &= \frac12 \sin\left(\frac{4h}{\omega}\right)\mathcal{J}_0\Big(\frac{4h}{\omega}\Big)\label{eq:sincos2}
\end{align}
Now, we consider asymptotic form of Bessel function for large $x$,
\begin{equation*}
	\mathcal{J}_\alpha(x)\approx\sqrt{\frac{2}{\pi x}}\cos \left(x-\frac{\alpha \pi}{2}-\frac{\pi}{4}\right).
\end{equation*}
When $x$ is chosen to lie near one of the roots of $\mathcal{J}_0(x)$ \textit{in the asymptotic limit of large-$x$}, then $x\approx [(2n+1)+1/2]\pi/2$. This yields asymptotic forms $\mathcal{J}_1(x)\sim x^{-1/2}/2, \mathcal{J}_2(x)\sim x^{-1/2}/2, \dots$. So, one can neglect $\mathcal{J}_{n>0}(x)$ in comparison to $\mathcal{J}_{0}(x)$ near the high roots. Substituting into Eq.\eqref{eq:hmovap1} finally yields the RWA-Hamiltonian ($\hat{H^{_{RWA}}}$),
\begin{multline}
    \hat{H}^{mov}\approx \hat{H}^{_{RWA}} = \frac{\hbar}{2}\sum_{ij} J_{ij} \Big(\hat{\sigma}^y_i\hat{\sigma}^y_j\Big) \mathcal{J}_0\Big(\frac{4h}{\omega}\Big)\Bigg\{\mathbbm{1}\bigg[\mathcal{J}_0\Big(\frac{4h}{\omega}\Big) + \cos\left(\frac{4h}{\omega}\right)\bigg] \\
    -\hat{\sigma}^z_i\hat{\sigma}^z_j \bigg[\mathcal{J}_0\Big(\frac{4h}{\omega}\Big) - \cos\left(\frac{4h}{\omega}\right)\bigg] + i \left(\hat{\sigma}^z_i + \hat{\sigma}^z_j\right) \sin\left(\frac{4h}{\omega}\right)\Bigg\}.
    \label{eq:hrwa}
