...
T = 2 * np.pi/w
steps = 20001
times = np.linspace(0, 2000 * T, steps, endpoint=False)

ea, eb = 0.03, 0.9

eaeb = np.array([[0.03, 0.9],[0.03, 0.7], [0.03, 0.3], [0.03,.1], [0.1, 0.11],[0.4, 0.1], [0.7, 0.1], [0.3, 0.31],[0.5, 0.51], [.8,.81]])
...
for hh,hpt in enumerate(hpts):
    ...
    for aa in eaeb:
        ...
        fname = "localmz_N"+str(N)+"2000T_ea_"+str(ea) + '_eb_'+ str(eb)+ '_J_'+ str(Jvalue)+".hdf5"
        with h5py.File(fname, 'w') as hf:
            ...
            hf.attrs['ea'] = ea
            hf.attrs['eb'] = eb
        print('time taken-', (time.time()-startt)/60, 'minutes')